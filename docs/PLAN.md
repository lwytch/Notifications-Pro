...
- [x] Portable asset-path exports/profiles: stop serializing machine-specific absolute asset paths for custom sounds/icons/backgrounds in shared settings exports and saved profiles; migrate toward managed relative asset identifiers so shared JSON is privacy-safer and more portable.
...
- [x] Architecture/status doc refresh: re-audited `docs/ARCHITECTURE.md` and `docs/STATUS.md` against the shipped codebase, refreshed stale component/storage/privacy/build-verification claims, and aligned the tracked docs with the current public project-level workflow plus the `1.1.10.34` verification state.
- [x] Documentation-maintenance guard skills: added local `maintain-architecture-docs` and `maintain-status-docs` skills so future agents explicitly recheck architecture/status drift whenever runtime shape, packaging/trust flow, defaults, privacy wording, or verification guidance changes.
- [x] Background-image runtime hardening: bound the transformed background-image cache so long Settings/Appearance editing sessions cannot retain unbounded processed bitmaps, reuse the managed asset-path helper for background-image path confinement parity, and add regression coverage for cache limits plus sibling-directory path rejection.
- [x] Privacy-disclosure accuracy pass: align Session Archive tray/settings wording and tracked docs (`SECURITY.md`, `README.md`, `docs/STATUS.md`, `CHANGELOG.md`) with the shipped clipboard behavior and the persisted app-name-derived settings/profile data.
- [x] Local settings/theme load hardening: add the same 1 MB guardrails used by explicit import flows to normal `settings.json` and custom-theme loads so oversized/tampered local files fail closed instead of causing avoidable startup/settings-window pressure, and cover the behavior with tests.
- [x] Release v1.1.10.34 hardening pass: updated README release notes plus repo-map/status/changelog planning docs, rebuilt a fresh signed MSIX, reran build/test/vulnerability checks, and completed local install verification with the replacement signing cert trusted on the review machine.
- [x] Release v1.1.10.36: fixed Settings-window live-save repositioning on multi-monitor setups, refreshed the public docs/release notes, reran build/test/repo-safety checks, and rebuilt plus installed a fresh signed MSIX for review.
