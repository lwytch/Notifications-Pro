...
- Overlay scrollbars now start disabled by default for a cleaner first-run overlay, and turning the toggle on updates the live overlay reliably and keeps the themed scrollbar visible instead of waiting for WPF `Auto` visibility
